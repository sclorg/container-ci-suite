...
class TestContainerIntegration:
    ...
    def test_ct_create_container_success(self, initialized_container_test_lib, mock_docker_commands, mock_file_operations):
    ...
    def test_ct_assert_container_creation_fails_success(self, initialized_container_test_lib, mock_docker_commands, mock_file_operations):
    ...
    def test_container_lifecycle(self, initialized_container_test_lib, mock_docker_commands, mock_file_operations):
...
class TestImageIntegration:
    ...
    def test_image_build_and_cleanup(self, initialized_container_test_lib, temp_dir, sample_dockerfile_content):
...
class TestEnvironmentIntegration:
    ...
    def test_ct_check_exec_env_vars_success(self, initialized_container_test_lib, sample_environment_vars):
    ...
    def test_ct_check_scl_enable_vars_success(self, initialized_container_test_lib, sample_environment_vars):
...
class TestDocumentationIntegration:
    ...
    def test_ct_doc_content_old_success(self, container_test_lib):
...
"""
    ...
    def test_ct_doc_content_old_missing_string(self, container_test_lib):
...
"""
    ...
    def test_ct_doc_content_old_invalid_format(self, container_test_lib):
...
class TestTestSuiteIntegration:
    """Integration tests for test suite functionality."""
    
    def test_ct_run_tests_from_testset(self, initialized_container_test_lib, capsys):
        """Test running a test set."""
        ct_lib = initialized_container_test_lib
        ct_lib.image_name = "test:latest"
        ...
        with patch.object(ct_lib, 'ct_timestamp_s', side_effect=[1000, 1030, 1060, 1090]), \
             patch.object(ct_lib, 'ct_timestamp_pretty', return_value="2024-01-01 12:00:00"):
            
            ct_lib.ct_run_tests_from_testset("testapp", test_set)
        
        assert "[PASSED]" in ct_lib.test_summary
        assert "[FAILED]" in ct_lib.test_summary
        assert ct_lib.testsuite_result == 1  # Failed because of test_failure
    
    def test_ct_run_tests_unstable_ignored(self, initialized_container_test_lib, clean_environment):
        """Test running tests with unstable tests ignored."""
        import os
        os.environ['IGNORE_UNSTABLE_TESTS'] = '1'
        
        ct_lib = initialized_container_test_lib
        ct_lib.image_name = "test:latest"
        ct_lib.unstable_tests = ["test_unstable"]
        
        def test_unstable():
            return False  # This test fails but should be ignored
        
        test_set = [test_unstable]
        
        with patch.object(ct_lib, 'ct_timestamp_s', side_effect=[1000, 1030]), \
             patch.object(ct_lib, 'ct_timestamp_pretty', return_value="2024-01-01 12:00:00"):
            
            ct_lib.ct_run_tests_from_testset("testapp", test_set)
        
        assert "[FAILED][UNSTABLE-IGNORED]" in ct_lib.test_summary
        assert ct_lib.testsuite_result == 0  # Should not fail because unstable test is ignored
...
class TestCleanupIntegration:
    ...
    def test_full_cleanup_cycle(self, initialized_container_test_lib, mock_docker_commands, mock_file_operations):
        """Test complete cleanup cycle."""
        ct_lib = initialized_container_test_lib
        
        # Create mock files
        app_id_file = ct_lib.app_id_file_dir / "test_image"
        cid_file = ct_lib.cid_file_dir / "test_container"
        
        mock_file_operations[str(app_id_file)] = "image123"
        mock_file_operations[str(cid_file)] = "container123"
        ...
        with patch('pathlib.Path.glob') as mock_glob, \
            ...
            mock_glob.side_effect = [
                [Path(str(app_id_file))],  # app_id_file_dir glob
                [Path(str(cid_file))]      # cid_file_dir glob
            ]
