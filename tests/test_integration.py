...
class TestContainerIntegration:
    ...
    def test_create_container_success(self, mock_docker_commands, mock_file_operations):
    ...
    #         result = self.lib.assert_container_creation_fails(
    #             cid_file_name="test_container",
    #             container_args="--invalid-arg",
    #             command="sleep 1",
    #         )
    #         assert result is True
...
class TestImageIntegration:
    ...
    def test_image_build_and_cleanup(self, temp_dir, sample_dockerfile_content):
...
class TestEnvironmentIntegration:
    ...
    def test_check_exec_env_vars_success(self, sample_environment_vars):
    ...
    def test_ct_check_scl_enable_vars_success(self, sample_environment_vars):
        ...
        with patch(
            ...
            mock_cmd.side_effect = [
                ...
                "\n".join(
                    [f"{k}={v}" for k, v in sample_environment_vars.items()]
                ),  # env output
                "\n".join(
                    [f"{k}={v}{v}" for k, v in sample_environment_vars.items()]
                ),  # scl enable env
            ]
...
class TestNetworkIntegration:
    ...
    def setup_method(self):
        """
        Create a fresh ContainerTestLib instance and assign it to self.lib for use by each test method.
        """
        self.lib = ContainerTestLib()
...
class TestNpmIntegration:
    ...
    def setup_method(self):
        """
        Initialize the test fixture by creating a ContainerTestLib instance and assigning it to `self.lib`.
        """
        self.lib = ContainerTestLib()
    ...
    def test_npm_works_success(self, temp_dir):
        ...
        with (
            ...
            patch(
                "container_ci_suite.utils.get_file_content", return_value="container123"
            ),
        ):
            mock_cmd.side_effect = [
                "8.19.2",  # npm --version
